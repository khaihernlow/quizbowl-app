...
const getQuestion = () => {
  ...
  const questions = [
    ...
    },
    {
      ...
      "tossup_question": "An aqueous solution in which the concentration of OH- ions is greater than that of H+ ions is:\nW) basic\nX) acidic\nY) neutral\nZ) in equilibrium",
    },
    {
      "category": "PHYSICS",
  ...
  answer = questions[counter].tossup_answer;
  if (counter === 4) {
    counter = 0;
  } else {
    counter += 1;
  ...
  const readEndTime = new Date(
    now.getTime() + 80 * tossup_question.split(' ').length
  );
